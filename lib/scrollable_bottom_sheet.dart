...
import 'package:scrollable_bottom_sheet/BottomSheetCustomPersistentHeader.dart';
import 'package:scrollable_bottom_sheet/BottomSheetState.dart';

class ScrollableBottomSheetWidget extends StatefulWidget {
  ScrollableBottomSheetWidget({Key? key, required this.body, required this.header}) : super(key: key);

  final Widget header;
  final Widget body;

  @override
  _ScrollableBottomSheetWidgetState createState() => _ScrollableBottomSheetWidgetState();
}

class _ScrollableBottomSheetWidgetState extends State<ScrollableBottomSheetWidget> {
  ValueNotifier<double> _topRadius = ValueNotifier(25);
  double _scrollLastExtent = 0.0;

  @override
  void initState() {
    _topRadius = ValueNotifier(MediaQuery.of(context).size.width * 0.07);
    super.initState();
  }
